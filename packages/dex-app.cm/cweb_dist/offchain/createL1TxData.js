import { CallType } from './shared';
const AMOUNT_BYTE_LENGTH = 16;
const calculateHexLength = (value) => (value.slice(0, 2).toLowerCase() === '0x' ? value.slice(2) : value).length;
...
const numberToOneByteUint8Array = (value) => {
...
const validateMaxAmount = (amount) => hexToUint8Array(amount).byteLength <= AMOUNT_BYTE_LENGTH;
export const createL1TxData = (params) => {
    ...
    if (params.callType === CallType.Transfer) {
        ...
        results.push(...numberToOneByteUint8Array(calculateHexLength(params.nextContractId)), ...nextContractId, ...nextContractMethod, ...numberToOneByteUint8Array(calculateHexLength(params.fallbackContractId)), ...fallbackContractId, ...fallbackContractMethod);
    }
    return new Uint8Array(results);
};
export const parseL1TxData = (hexData) => {
    const hex = hexData.slice(0, 2).toLowerCase() === '0x' ? hexData : `0x${hexData}`;
    const binString = BigInt(hex).toString(2);
    let offset = 0;
    const sliceNext = (length) => {
    ...
    const result = {};
    result.callType = bin8StringToNumber(sliceNext(8));
...
export const uint8ArrayToHexStr = (uint8) => {
