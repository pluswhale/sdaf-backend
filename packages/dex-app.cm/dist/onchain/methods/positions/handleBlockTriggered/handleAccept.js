import { addContinuation, constructContinueTx, constructContractRef, constructRead, constructStore, constructTake, passCwebFrom, } from '@coinweb/contract-kit';
...
import { constructSendCweb, createCallWithL1EventBlock, createClosedIndexClaim, createFundsClaim, createPositionStateClaim, constructJumpCall, createBestByQuoteIndex, getInstanceParameters, createEvmEventClaimKey, constructConditional, } from '../../../utils';
const constructTotalAccept = (context, issuer, positionId, positionState, sendAmount, cwebAccount) => {
...
const constructAcceptWithNewPosition = (context, issuer, positionId, nonce, positionState, sendAmount, positionCwebAmount, cwebAccount, fundsClaim, availableCweb, authenticated) => {
...
const constructAcceptWithCwebReturn = (context, issuer, positionId, positionState, sendAmount, positionCwebAmount, cwebAccount, fundsClaim, availableCweb) => {
...
export const handleAccept = (context, issuer, positionId, nonce, positionState, positionStoredAmount, fundsClaim, availableCweb, authenticated, paidAmount, cwebAccount, recipient) => {
    ...
    if (isParticular) {
        if (isEvmL1Type) {
            return constructAcceptWithNewPosition(context, issuer, positionId, nonce, positionState, dueCwebAmount, positionCwebAmount, cwebAccount, fundsClaim, availableCweb, authenticated);
        }
        else {
            return constructAcceptWithCwebReturn(context, issuer, positionId, positionState, dueCwebAmount, positionCwebAmount, cwebAccount, fundsClaim, availableCweb);
        }
    }
