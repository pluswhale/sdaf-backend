import { getContractId, constructContractIssuer, getMethodArguments, extractContractInfo, extractContractArgs, extractRead, } from '@coinweb/contract-kit';
import { CallType } from '../../../../offchain/shared';
...
export const handleBlockTriggered = (context) => {
    const { tx } = context;
    ...
    const [, positionId, nonce] = getMethodArguments(context);
    ...
    const positionStateClaim = extractRead(contractArgs[2])?.[0]?.content;
    ...
    if (eventClaim) {
        ...
        if (eventData.callType === CallType.Transfer) {
            return handleTransfer(context, issuer, positionId, nonce, positionState, positionStoredAmount, positionFundsClaim, availableCweb, auth, eventData.paidAmount, eventData.recipient, {
        ...
        return handleAccept(context, issuer, positionId, nonce, positionState, positionStoredAmount, positionFundsClaim, availableCweb, auth, eventData.paidAmount, 
        //TODO! Change event data to support user
        {
            auth: 'EcdsaContract',
            payload: eventData.baseRecipient,
        }, satisfies, User, eventData.recipient);
    }
