import { addContinuation, constructContinueTx, constructContractIssuer, constructContractRef, constructRead, constructStore, constructTake, passCwebFrom, prepareQueueCall, queueCostFee, } from '@coinweb/contract-kit';
...
import { createCallWithL1EventBlock, createClosedIndexClaim, createFundsClaim, createPositionStateClaim, constructJumpCall, createBestByQuoteIndex, getInstanceParameters, createEvmEventClaimKey, constructConditional, constructSendCweb, log, } from '../../../utils';
const constructTransfer = (context, issuer, positionId, positionState, amount, callContractData) => {
...
const constructTransferWithNewPosition = (context, issuer, positionId, nonce, positionState, spendAmount, positionCwebAmount, fundsClaim, availableCweb, authenticated, callContractData) => {
    const { l2Contract, l2MethodName, l2Args } = callContractData;
    ...
    return [
        constructContinueTx(context, [
            ...
            constructStore(createFundsClaim({
                ...fundsClaim.body,
                positionId,
...
const constructTransferWithCwebReturn = (context, issuer, positionId, positionState, spendAmount, positionCwebAmount, fundsClaim, availableCweb, callContractData) => {
    const { l2Contract, l2MethodName, l2Args } = callContractData;
    const transactionFee = 4000n + queueCostFee();
...
export const handleTransfer = (context, issuer, positionId, nonce, positionState, positionStoredAmount, fundsClaim, availableCweb, authenticated, paidAmount, recipient, callContractData) => {
    ...
    if (isParticular) {
        if (isEvmL1Type) {
            return constructTransferWithNewPosition(context, issuer, positionId, nonce, positionState, dueCwebAmount, positionCwebAmount, fundsClaim, availableCweb, authenticated, callContractData);
        }
        else {
            return constructTransferWithCwebReturn(context, issuer, positionId, positionState, dueCwebAmount, positionCwebAmount, fundsClaim, availableCweb, callContractData);
        }
    }
