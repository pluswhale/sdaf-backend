import { constructBlock, constructContinueTx, constructContractIssuer, constructRead, constructStore, constructTake, extractContractArgs, extractContractInfo, extractRead, extractUser, getContractId, getMethodArguments, getQueueAuthenticated, prepareQueueCall, queueCostFee, } from '@coinweb/contract-kit';
import { ACTIVITY_STATUS, createActivePositionIndexKey, createBestByQuoteActiveIndexKey, createPositionFundsKey, } from '../../../../offchain/shared';
import { CONSTANTS, PRIVATE_METHODS } from '../../../constants';
import { constructConditional, createClosedPositionBlockFilter, createExpirationPositionBlockFilter, createPositionStateClaim, createBestByQuoteIndex, getTime, isEqualUser, } from '../../../utils';
export const deactivatePosition = (context) => {
    const { tx } = context;
    const { providedCweb: availableCweb, authenticated: auth } = extractContractInfo(tx);
    if (!availableCweb) {
        throw new Error('Cweb was not provided');
    }
    const [, positionId] = getMethodArguments(context);
    const contractArgs = extractContractArgs(tx);
    const positionClaim = extractRead(contractArgs[0])?.[0]?.content;
    if (!positionClaim) {
        throw new Error('Position does not exist');
    }
    const positionFundsClaim = extractRead(contractArgs[1])?.[0]?.content;
    ...
    return [
        constructContinueTx(context, [
            ...
            constructStore(createPositionStateClaim({
                ...
                body: {
                    ...positionState,
                    expirationDate: closingPlannedDate < positionState.expirationDate ? closingPlannedDate : positionState.expirationDate,
                    activityStatus: ACTIVITY_STATUS.CANCELLING,
                },
            })),
        ]),
        ...constructConditional(closingPlannedDate < positionState.expirationDate, constructContinueTx(context, [
            constructBlock([
