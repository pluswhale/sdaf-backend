import { getContractId, constructContinueTx, constructContractIssuer, getMethodArguments, extractContractInfo, extractContractArgs, extractRead, passCwebFrom, constructStore, constructClaimKey, } from '@coinweb/contract-kit';
import { PRIVATE_METHODS } from '../../../constants';
import { L1Types } from '../../../types';
import { constructConditional, getUser, constructSendCweb, createActiveIndexClaim, createBestByQuoteIndexClaim, createCallWithL1EventBlock, createCreatePositionCallPrivate, createDateIndexClaim, createFundsClaim, createPositionStateClaim, createUserIndexClaim, hashClaimBody, constructJumpCall, createBestByQuoteActiveIndexClaim, getInstanceParameters, validateBtcChainData, createEvmEventClaimKey, } from '../../../utils';
export const createPosition = (context) => {
    const { tx } = context;
    ...
    const [, positionId, positionState, ownerFee] = getMethodArguments(context);
    ...
    if (existingPosition) {
        ...
        return createCreatePositionCallPrivate(context, issuer, positionNewId, [positionNewId, positionState, ownerFee], availableCweb - transactionFee, auth);
    }
    ...
    return [
        constructContinueTx(context, [
            ...
            constructStore(createActiveIndexClaim({
                timestamp: positionState.createdAt,
                positionId,
            })),
            constructStore(createBestByQuoteActiveIndexClaim({
                baseAmount,
                quoteAmount,
                positionId,
            })),
            constructStore(createDateIndexClaim({
                timestamp: positionState.createdAt,
                positionId,
            })),
            constructStore(createBestByQuoteIndexClaim({
                baseAmount,
                quoteAmount,
                positionId,
            })),
            ...constructConditional(signer.auth === 'EcdsaContract', constructStore(createUserIndexClaim({
                user: signer,
                timestamp: positionState.createdAt,
                positionId,
            }))),
