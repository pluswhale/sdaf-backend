...
import { createOrderUiCommand, deleteOrderUiCommand, makeDepositUiCommand, makeWithdrawUiCommand, } from 'market-maker.cm';
...
export function contractOwnerFee(OWNER_MIN_FEE, OWNER_PERCENTAGE_FEE, fromValue) {
    const minFee = BigInt(OWNER_MIN_FEE || 0);
    if (!OWNER_PERCENTAGE_FEE) {
        return minFee;
    }
    const calculatedOwnerPercentageFee = (BigInt((Number(fromValue) || 0) * Number(OWNER_PERCENTAGE_FEE)) * BigInt(1e18)) / 100n;
...
export async function convertInfoToCreateQr(symbol, ethDest, amount, price, chainData) {
    ...
    if (currency1 === Currency.CWEB) {
        ...
        contractParam.contractOwnerFee = toHex(contractOwnerFee(currency2 !== 'CWEB' ? CONTRACT_PARAMS[currency2].L2_OWNER_MIN_FEE_BASE : '', currency2 !== 'CWEB' ? CONTRACT_PARAMS[currency2].L2_OWNER_PERCENTAGE_FEE_BASE : ''));
...
export function convertInfoToCancelQr(positionId, symbol) {
    const { currency1, currency2 } = getCurrencyForPair(symbol);
    if (currency1 === Currency.CWEB) {
...
export function convertInfoToTransfer(fromAccount, toAccount, amount) {
