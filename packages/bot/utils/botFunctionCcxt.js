import { force_refresh as forceRefresh } from '@coinweb/wallet-lib';
...
const GLOBAL_WRONG_PACT = [];
async function findDifferences(filterData, projection, wallet, txMonitor, isC2) {
    console.log(wallet, 'wallet');
    console.log(txMonitor, 'txMonitor');
    let newProjection = [...projection];
    for (let i = 0; i < filterData.length; i += 1) {
        console.log(newProjection, 'newProjection');
        if (filterData[i]?.covering &&
        ...
        const elements = newProjection.filter((item) => {
            ...
            console.log((item.cweb * (100 + item.different)) / 100, 'cweb + different'); */
            return ((item.cweb * (100 - item.different)) / 100 <= Number(filterData[i].baseAmount) / 10 ** decimalsCweb &&
...
async function getBtc(root, utxos, wallet) {
    const publicKey = Buffer.from(root._publicKey).toString('hex');
    const { utxoId, vout, psbtB64 } = await getPsbtStartTransaction(wallet ?? '', publicKey, root._privateKey, utxos);
    return { utxoId, vout, psbtB64 };
}
export async function chandePositions(filterData, projection, wallet, txMonitor, ethWallet, root, usedUtxoMas) {
...
export async function chandeOrders(filterData, projection, wallet, collateralConst, txMonitor) {
    ...
    if (filterData?.length === 0) {
        ...
        for (let i = 0; i < projection.length; i += 1) {
            const isNewCollateralBalance = await checkCollateral(balance, projection[i].cweb * collateralConst, wallet, `collateral_${projection[i].token.toLowerCase()}`, txMonitor);
    ...
    if (newOrders.length > 0) {
        ...
        for (let i = 0; i < newOrders.length; i += 1) {
            const isNewCollateralBalance = await checkCollateral(balance, newOrders[i].cweb * collateralConst, wallet, `collateral_${newOrders[i].token.toLowerCase()}`, txMonitor);
            if (isNewCollateralBalance) {
                balance = isNewCollateralBalance;
                continue;
            }
...
async function checkCollateral(balance, cweb, wallet, toAccount, txMonitor) {
...
async function cancelOrder(id, symbol, wallet, txMonitor) {
    try {
...
async function createOrder(symbol, amount, price, wallet, txMonitor, ethWallet, chainData) {
    try {
...
async function approveERC20(signer, item, token) {
    const { L1_TOKEN_ADDRESS, L1_CONTRACT_ADDRESS_MAKER } = ERC20_TOKENS.includes(token)
        ? CONTRACT_PARAMS[token]
