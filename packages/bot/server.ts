    ...
    try {
      ...
      if (useC1) {
        ...
        if (currency === Currency.BTC) {
          const usedUtxoOrder = dataC1?.filter(
            (item) => item.activityStatus === ACTIVITY_STATUS.ERROR && item.error === 'UTXO is already in use',
          );
          ...
          usedUtxoOrder?.forEach((order: any) => {
            if (
              !usedUtxoMas.find(
                (el) =>
                  el.l1TxId === (order?.chainData as BtcChainData)?.l1TxId &&
                  el.vout === (order?.chainData as BtcChainData)?.vout,
              )
            ) {
              usedUtxoMas.push({
