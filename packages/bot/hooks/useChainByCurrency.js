...
import { ERC20_TOKENS, EVM_TOKENS } from "../constants";
const useChainByCurrency = (currency) => {
    const evmWagmiChains = useChains();
    const collectChainData = (currency) => {
        ...
        const chainData = evmWagmiChains.find((chain) => chain.nativeCurrency.symbol.toUpperCase() === (tokenChain || token).toUpperCase());
        return {
            isNative,
            isEvm,
            isERC20,
