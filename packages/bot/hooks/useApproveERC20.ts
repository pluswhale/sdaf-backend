...
const useApproveERC20 = () => {
  ...
  const getErc20Abi = (tokenAddress: EvmContractAddress) => {
    ...
    if (getContractParams(Currency.USDT_ETH).L1_TOKEN_ADDRESS.toLowerCase() === tokenAddress.toLowerCase()) {
      ...
      return erc20Abi.map((abiPartial) => {
        if (abiPartial.name === 'approve') return { ...abiPartial, outputs: [] };
  ...
  return {
    ...rest,
    isLoading: isPending,
    approve: (tokenAddress: EvmContractAddress, spender: EvmContractAddress, amount: bigint) => {
