...
export function decimalsForCurrency(currency) {
    switch (currency) {
        case Currency.BTC:
            return 8;
...
export function convertStringToBigInt(value, currency) {
...
export function convertBigIntToString(value, currency, roundedTo) {
    const decimals = decimalsForCurrency(currency);
...
export const humanReadableStringFromTokenBigInt = (amount, token, precision = 10) => {
    return formatNumberWithPrecision(Number(convertBigIntToString(amount, token)), precision);
};
export function estimatedToReceive(inputCurrency, outputCurrency, inputValue) {
    ...
    if (!conversion) {
        if (inputCurrency !== Currency.USDT_ETH && outputCurrency !== Currency.USDT_ETH) {
            ...
            if (inputCurrency !== Currency.USDT_BNB && outputCurrency !== Currency.USDT_BNB) {
                ...
                throw new Error(`Unsupported currency pair ${inputCurrency}-${outputCurrency}`);
        }
    }
    return {
...
export function formatCurrencyNumber(amount, decimals) {
...
export function formatCurrency(currency, amount, withCurrency = true) {
    const decimals = decimalsForCurrency(currency);
