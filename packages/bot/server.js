...
async function getTokenPrice(token) {
    ...
    if (token === Currency.USDT_BNB || token === Currency.USDT_ETH)
        return 1;
...
async function getValuePositions(token, isC2) {
    let end = endValue;
...
async function botWork(wallet, txMonitor) {
    ...
    for (let i = 0; i < curMas.length; i++) {
        try {
            ...
            if (usePact) {
                ...
                claims.forEach(async (claim, index) => {
...
async function intervalBot(wallet, txMonitor) {
