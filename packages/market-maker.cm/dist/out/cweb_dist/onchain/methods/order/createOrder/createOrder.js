import { constructBlock, constructContinueTx, constructRead, constructStore, constructTake, passCwebFrom, prepareQueueCall, queueCostFee, } from '@coinweb/contract-kit';
import { ORDER_ACTIVITY_STATUS, createOrderStateKey, createMakerDepositKey, createOrderCollateralKey, } from '../../../../offchain/shared/index.js';
import { CONSTANTS, PRIVATE_METHODS } from '../../../constants.js';
import { createBestOrderIndexClaim, createClosedOrderBlockFilter, createExpirationBlockFilter, createOrderActiveIndexClaim, createOrderCollateralClaim, createOrderDateIndexClaim, createOrderByOwnerIndexClaim, createOrderStateClaim, createMakerDepositClaim, getCallParameters, getContractIssuer, getMethodArguments, getReadClaimByIndex, getUser, hashObject, createBestActiveOrderIndexClaim, } from '../../../utils/index.js';
...
export const createOrder = (context) => {
    ...
    const [id, initialState] = getMethodArguments(context);
    const issuer = getContractIssuer(context);
    const existingOrder = getReadClaimByIndex(context)(0);
    if (existingOrder) {
    ...
    const depositClaim = getReadClaimByIndex(context)(1);
    ...
    return [
        constructContinueTx(context, [
            ...
            constructStore(createOrderActiveIndexClaim({
                id,
            ...
            constructStore(createOrderDateIndexClaim({
                id,
                timestamp: initialState.createdAt,
            })),
            constructStore(createBestOrderIndexClaim({
                id,
                baseAmount,
                quoteAmount,
            })),
            constructStore(createOrderByOwnerIndexClaim({
                id,
                user,
                timestamp: initialState.createdAt,
            })),
        ...
        ], [
            {
                callInfo: prepareQueueCall(getContractIssuer(context), {
                    methodInfo: {
                        ...
                        methodArgs: [id, ORDER_ACTIVITY_STATUS.EXPIRED],
                    },
                    contractInfo: {
