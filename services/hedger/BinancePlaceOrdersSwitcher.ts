...
export const BinancePlaceOrdersSwitcher = async (fromCoin: string, toCoin: string, transaction: any, direction: string, amount: string | number, bestOrder: any, targetWalletAddress: string): Promise<HeObjectForSavingInDb> => {
  ...
  switch (pair) {
    case 'BTCUSDT': {
      ...
      generatedObjectForSavingInDB.margin = await getMarginByPrice(String(generatedObjectForSavingInDB.priceHedgedOnBinance));
      generatedObjectForSavingInDB.priceSettledToUser = String(+bestOrder?.[0] - +bestOrder?.[0] * Number(generatedObjectForSavingInDB.margin?.marginValue)); //needs to come from bot
      ...
      const profitFromSwap = Math.abs(+amount * +generatedObjectForSavingInDB.priceHedgedOnBinance - +amount * +generatedObjectForSavingInDB.priceSettledToUser);
